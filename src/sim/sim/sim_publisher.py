...
def PF_A_name(names):
    # 假设 self.sim_.m_ 就是 mjModel* 的 python 绑定
    model_names_bytes = names  # 一般是个 bytes 对象，里面存放了所有名称
    if isinstance(model_names_bytes, bytes):
        # 以 '\0' 作为分隔符，只在第一个 '\0' 处分割
        splitted = model_names_bytes.split(b'\x00', 1)
        if splitted and splitted[0]:
            model_name = splitted[0].decode('utf-8')  # 得到 "PF_A"
        else:
            model_name = "NoName"
    else:
        # 如果不是 bytes，就转成字符串再做处理
        model_name = str(model_names_bytes)
...
class SimPublisher(Node):
    ...
    def __init__(self, sim: Simulate, config_yaml: str):
        ...
        # print("actuators_cmds_topic",actuators_cmds_topic)  
    ...
    def reset_callback(self, request: SimulationReset.Request, response: SimulationReset.Response):
        ...
        if self.sim_.d_ is not None:
            if request.header.frame_id != PF_A_name(self.sim_.m_.names):
                self.get_logger().error(f"reset request is not for {PF_A_name(self.sim_.m_.names)}")
                ...
                with self.sim_.mtx:
                    ...
                    with self.actuator_cmds_buffer_.mtx:
                        print("actuators_name",self.actuator_cmds_buffer_.actuators_name)
    ...
    def imu_callback(self):
        ...
        if self.sim_.d_ is not None:
            message = Imu()
            frame_id_value = PF_A_name(self.sim_.m_.names)
            # 确保 frame_id 是字符串类型
            if not isinstance(frame_id_value, str):
                self.get_logger().warn(f"frame_id_value is not a string: {frame_id_value} (type: {type(frame_id_value)})")
                frame_id_value = str(frame_id_value)

            message.header.frame_id = frame_id_value
    ...
    def odom_callback(self):
        ...
        if self.sim_.d_ is not None:
            message = Odometry()
            with self.sim_.mtx:
                message.header.frame_id = PF_A_name(self.sim_.m_.names)
                message.header.stamp = self.get_clock().now().to_msg()
                # 设置位置和姿态
    ...
    def joint_callback(self):
        ...
        if self.sim_.d_ is not None:
            ...
            jointState.header.frame_id = PF_A_name(self.sim_.m_.names)
            ...
            with self.sim_.mtx:
                for i in range(self.sim_.m_.njnt):
                    if self.sim_.m_.jnt_type[i] != mujoco.mjtJoint.mjJNT_FREE:
                        jnt_name = mujoco.mj_id2name(self.sim_.m_, mujoco.mjtObj.mjOBJ_JOINT, i)
    ...
    def actuator_cmd_callback(self, msg: ActuatorCmds):
        ...
        """       
        # self.get_logger().info(f"subscribe actuator cmds time:  {self.actuator_cmds_buffer_.time}")
        ...
        with self.actuator_cmds_buffer_.mtx:
            ...
            if abs(self.actuator_cmds_buffer_.time - current_time) > 0.2:
                for k in range(len(self.actuator_cmds_buffer_.actuators_name)):
                    self.actuator_cmds_buffer_.kp[k] = 0.0
                    self.actuator_cmds_buffer_.pos[k] = 0.0
                    self.actuator_cmds_buffer_.kd[k] = 1.0
                    self.actuator_cmds_buffer_.vel[k] = 0.0
                    self.actuator_cmds_buffer_.torque[k] = 0.0
    ...
    def __del__(self):
        ...
        析构函数。
        """
        self.get_logger().info("Close SimPublisher node ...")
